...
export class ChatHandler {
  ...
  private async generateToolResponse(
    ...
    // A special case for simulation results to return structured JSON for the frontend
    const simResult = toolResults.find(r => r.name === 'simulate_campaign' && typeof r.result === 'object' && r.result && 'roi' in r.result);
    if (simResult) {
      return JSON.stringify({
        component: 'simulation_results',
        props: simResult.result
      });
    }
    ...
    return [
      {
        ...
        content: `You are InsightAgent, an expert e-commerce campaign analyst. Your goal is to help users analyze past campaign performance and simulate future ones.
- When a user wants to simulate a campaign, you MUST use the 'simulate_campaign' tool.
- Before calling the tool, if you don't have all the required parameters (budget, promo_type, channel_mix), you MUST ask the user for the missing information by responding with a specific JSON structure: \`{"component": "simulation_form"}\`. Do not ask in plain text.
- Once you have all parameters, call the 'simulate_campaign' tool.
- When you receive the results from the 'simulate_campaign' tool, you MUST present them to the user by responding with the structured JSON you received from the tool. Do not add any conversational text around it.`
      },
