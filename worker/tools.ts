...
const customTools = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Simulates an e-commerce campaign to predict its performance.',
      parameters: {
        type: 'object',
        properties: {
          budget: { type: 'number', description: 'The total budget for the campaign in USD.' },
          promo_type: { type: 'string', enum: ['percentage_discount', 'bogo', 'free_shipping'], description: 'The type of promotion to be used.' },
          channel_mix: {
            type: 'object',
            properties: {
              instagram: { type: 'number', description: 'Percentage of budget allocated to Instagram (0-100).' },
              google_ads: { type: 'number', description: 'Percentage of budget allocated to Google Ads (0-100).' },
              email: { type: 'number', description: 'Percentage of budget allocated to Email marketing (0-100).' },
            },
            required: ['instagram', 'google_ads', 'email'],
          },
        },
        required: ['budget', 'promo_type', 'channel_mix'],
      },
    },
  },
];
  ...
  try {
    switch (name) {
      ...
      case 'simulate_campaign': {
        const { budget, promo_type, channel_mix } = args;
        // Mock logic for simulation
        const promoMultiplier = { percentage_discount: 1.2, bogo: 1.5, free_shipping: 1.1 };
        const revenue = budget * (Math.random() * 2 + 2) * (promoMultiplier[promo_type] || 1);
        const roi = (revenue - budget) / budget;
        const sales_lift = (roi * 100) / 5 + (Math.random() * 10);
        const totalMix = channel_mix.instagram + channel_mix.google_ads + channel_mix.email;
        const channel_performance = [
          { name: 'Instagram', value: Math.round(revenue * (channel_mix.instagram / totalMix)) },
          { name: 'Google Ads', value: Math.round(revenue * (channel_mix.google_ads / totalMix)) },
          { name: 'Email', value: Math.round(revenue * (channel_mix.email / totalMix)) },
        ];
        return {
          roi: roi,
